 ...
 */
@Slf4j
@Service("simpleShearTextCaptchaService")
public class CaptchaSimpleShearTextServiceImpl extends CaptchaBaseAbstractServiceImpl {
    ...
    protected CaptchaImageBO CaptchaImageBO(CaptchaCreateDTO createDTO, CaptchaViewDTO view) {
        ...
        Integer interferingLineCount = configBO.getInterferingLineCount() == null ? 4 : configBO.getInterferingLineCount();

        RandomGenerator randomGenerator = new RandomGenerator(TEXT_SOURCE, codeCount);

        ShearCaptcha captcha = new ShearCaptcha(width, height, codeCount, interferingLineCount);
